...
#include "ui_zzolexcoincontroldialog.h"
...
std::list<std::string> ZZolexCoinControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZolexCoinControlDialog::listMints;

ZZolexCoinControlDialog::ZZolexCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZolexCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZolexCoinControlDialog::~ZZolexCoinControlDialog()
{
    delete ui;
}

void ZZolexCoinControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZolexCoinControlDialog::updateList()
{
...
void ZZolexCoinControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZolexCoinControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZolexCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZolexCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZolexCoinControlDialog::GetSelectedMints()
{
...
void ZZolexCoinControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
