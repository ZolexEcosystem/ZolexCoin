...
#include "zzolexcoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZLX ought to be enough for anybody." - Bill Gates, 2017
    ui->zZLXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZLXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZLX->setEnabled(false);
        ui->pushButtonMintzZLX->setToolTip(tr("zZLX is currently disabled due to maintenance."));

        ui->pushButtonSpendzZLX->setEnabled(false);
        ui->pushButtonSpendzZLX->setToolTip(tr("zZLX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZLXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZLX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZLX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZLX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZLX in ") +
...
void PrivacyDialog::on_pushButtonSpendzZLX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZLX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zZLX
        sendzZLX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zZLX
    sendzZLX();
}

void PrivacyDialog::on_pushButtonZZolexCoinControl_clicked()
{
    ZZolexCoinControlDialog* zZLXControl = new ZZolexCoinControlDialog(this);
    zZLXControl->setModel(walletModel);
    zZLXControl->exec();
}

void PrivacyDialog::setZZolexCoinControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZLXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZLX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid ZolexCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zZLXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZLX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZOLEXCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZLX</b>";
    ...
    // use mints from zZLX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZZolexCoinControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZZolexCoinControlDialog::GetSelectedMints();
    }
    ...
    // Clear zzolexcoin selector in case it was used
    ZZolexCoinControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZLX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ZolexCoin, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZLX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZLX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZLX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZLX "));
    ui->labelzZLXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzZLXSyncStatus->setVisible(fShow);
}
